{ lib, stdenvNoCC, autoPatchelfHook, fetchurl, curl, alsa-lib, freetype, libgcc
, libGL, makeDesktopItem, copyDesktopItems }:
stdenvNoCC.mkDerivation (finalAttrs: {
  pname = "erae-lab";
  version = "1.4.1";
  src = fetchurl {
    url =
      "https://storage.googleapis.com/videohosting/DOWNLOAD_Ressources/ERAE_Software/EraeLab_${finalAttrs.version}";
  ...
  buildInputs = [ curl alsa-lib freetype libGL libgcc.lib ];
  postInstall = ''
    mkdir -p $out/bin
  ...
  desktopItems = [
    (makeDesktopItem {
      ...
      icon = fetchurl {
        url =
          "https://s3-eu-central-1.amazonaws.com/euc-cdn.freshdesk.com/data/helpdesk/attachments/production/80055164819/original/Ldm_LToocRwVP7BQLCGJ4bY6qgtve0i0lw.png";
