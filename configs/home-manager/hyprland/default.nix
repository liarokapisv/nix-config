{ config, lib, pkgs, ... }: {
  ...
  options = {
    wayland.windowManager.hyprland.ext = {
      keybinds.wireplumber.enable =
        lib.mkEnableOption "wireplumber volume keybinds";
    };
  };

  config = let cfg = config.wayland.windowManager.hyprland;
  ...
  in lib.mkIf (cfg.enable) {
    ...
    programs = {
      swww.systemd = { target = "hyprland-session.target"; };
